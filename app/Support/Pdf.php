 ...
 *
 * PORTED VERBATIM from server/src/pdf.php (decision D8). The body below is the
 * legacy class, renamed and namespaced and otherwise untouched, because a
 * payslip is a document people keep: swapping in dompdf or TCPDF would re-flow
 * every one of them, and "the same numbers in a different layout" is still a
 * visible change to somebody reading their own pay.
 *
 * Supports the base-14 Helvetica / Helvetica-Bold fonts (so nothing has to be
 * embedded), text with REAL advance widths — which is what makes right
 * alignment and wrapping land correctly — lines, filled rectangles, JPEG
 * images, and multi-page output with automatic page breaks.
 *
 * Coordinates are points from the TOP-LEFT of the page, which is far easier to
 * lay out with than PDF's native bottom-left origin; y() does the conversion.
 *
 * Only JPEG is supported for images, because DCTDecode lets the bytes through
 * untouched and no decoder is needed. Payslips::logoJpeg() converts the
 * organization's WebP logo on the way in.
 *
