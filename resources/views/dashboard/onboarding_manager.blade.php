...
{{--
    ...
    Ported from server/templates/dashboard/onboarding_manager.php.

    Two steps instead of the company admin's five: a manager cannot create
    clients, teams or accounts above their own level, so the other steps would
    be walls rather than work.
--}}
            ...
            <table class="data" style="margin-top:.6rem">
                ...
                <tbody>
                    @forelse ($roster as $agent)
                        <tr><td>{{ $agent->name }}</td><td>{{ $agent->email }}</td>
                            <td><span class="tag">{{ $agent->role->label() }}</span></td></tr>
                    @empty
        ...
        <div class="panel">
            ...
            @if (count($roster))
                <form method="post" action="{{ url('/app/onboarding') }}" class="onb-row">
                    ...
                    <select name="user_id">
                        <option value="{{ (int) $me->id }}">You ({{ $me->role->label() }})</option>
                        @foreach ($roster as $agent)
                            <option value="{{ (int) $agent->id }}">{{ $agent->name }} ({{ $agent->role->label() }})</option>
                        @endforeach
                    </select>
