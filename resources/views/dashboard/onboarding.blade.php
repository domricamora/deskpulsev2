...
{{--
    The company admin's setup wizard.
    Ported from server/templates/dashboard/onboarding.php.

    Five steps and a summary. Every form carries the step it was posted from,
    so the redirect lands back where the person was standing.
--}}
...
@php
    ...
    $roles = ['member' => 'Employee', 'manager' => 'Team manager',
              'hr_manager' => 'HR admin', 'it_admin' => 'IT admin'];

    $index = array_search($step, $steps, true);
    $next = $steps[$index + 1] ?? 'done';
    $previous = $index > 0 ? $steps[$index - 1] : null;
    $go = fn (string $s) => url('/app/onboarding?step=' . $s);

    // Tasks are for the people who do the work: agents and team managers.
    $assignees = $members->whereIn('role', [\App\Enums\UserRole::Member, \App\Enums\UserRole::Manager]);
@endphp
...
@section('content')
    ...
    <ol class="onb-steps">
        @foreach ($steps as $i => $s)
            <li class="{{ $s === $step ? 'on' : ($i < $index ? 'done' : '') }}">
                <a href="{{ $go($s) }}"><span class="n">{{ $i < $index ? '✓' : $i + 1 }}</span>{{ $labels[$s] }}</a>
            </li>
        @endforeach
    </ol>

    @if ($step !== 'done')
        <form method="post" action="{{ url('/app/onboarding') }}" class="onb-skip">
            @csrf
    ...
    @if ($step === 'org')
        <div class="panel">
            ...
            <form method="post" action="{{ url('/app/onboarding') }}" class="stack">
                @csrf
                <input type="hidden" name="step" value="org">
                <label>Organization name
                    <input type="text" name="name" value="{{ $org->name }}" maxlength="160" required></label>
                ...
                <label class="check"><input type="checkbox" name="screenshot_blur" @checked($policy['screenshot_blur'])> Blur screenshots (privacy)</label>
                <label class="check"><input type="checkbox" name="track_screenshots" @checked($policy['track_screenshots'])> Capture screenshots</label>
                <label class="check"><input type="checkbox" name="track_windows" @checked($policy['track_windows'])> Track active windows</label>
                <label class="check"><input type="checkbox" name="track_processes" @checked($policy['track_processes'])> Track running apps</label>
                <div><button class="btn" name="action" value="save_org">Save &amp; continue →</button></div>
            </form>
            <x-onboarding-nav :previous="$previous" :next="$next" />
        </div>
    ...
    @elseif ($step === 'clients')
        <div class="panel">
            ...
            <form method="post" action="{{ url('/app/onboarding') }}" class="onb-row">
                @csrf
            ...
            <ul class="onb-list">
                @forelse ($clients as $client)
                    <li><b>{{ $client->name }}</b>@if ($client->contact_email) <span class="muted">· {{ $client->contact_email }}</span>@endif
                        @if ($client->user_id) <span class="status approved">portal login</span>@endif</li>
                @empty
                    <li class="muted">No clients yet — add one above (optional).</li>
                @endforelse
            </ul>
            <x-onboarding-nav :previous="$previous" :next="$next" />
        </div>
        ...
        <div class="panel">
            ...
            <ul class="onb-list">
                @forelse ($teams as $team)
                    <li><b>{{ $team->name }}</b> <span class="muted">·
                        {{ ! empty($teamMembers[$team->id]) ? implode(', ', $teamMembers[$team->id]) : 'no members yet' }}</span></li>
                @empty
                    <li class="muted">No teams yet — add one above (optional).</li>
                @endforelse
            </ul>
            <x-onboarding-nav :previous="$previous" :next="$next" />
        </div>
            ...
            <table class="data" style="margin-top:.6rem">
                ...
                <tbody>
                    @forelse ($members as $member)
                        <tr><td>{{ $member->name }}</td><td>{{ $member->email }}</td>
                            <td><span class="tag">{{ $member->role->label() }}</span></td></tr>
                    @empty
                        <tr><td colspan="3" class="muted">No agents yet — add your first above.</td></tr>
                    @endforelse
                </tbody>
            </table>
            <x-onboarding-nav :previous="$previous" :next="$next" />
        ...
        <div class="panel">
                ...
                <form method="post" action="{{ url('/app/onboarding') }}" class="onb-row">
                    ...
                    <select name="user_id">
                        @foreach ($assignees as $assignee)
                            <option value="{{ (int) $assignee->id }}">{{ $assignee->name }} ({{ $assignee->role->label() }})</option>
                        @endforeach
                    </select>
            ...
            <ul class="onb-list">
                @forelse ($tasks as $task)
                    <li><b>{{ $task->title }}</b> <span class="muted">· {{ $task->owner }}@if ($task->client) · {{ $task->client }}@endif</span></li>
                @empty
                    <li class="muted">No tasks yet (optional).</li>
                @endforelse
            </ul>
            <div class="onb-nav">
    ...
    @else
        <div class="panel onb-done">
            ...
            <form method="post" action="{{ url('/app/onboarding') }}">
                @csrf
                <input type="hidden" name="step" value="done">
