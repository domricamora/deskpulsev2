...
| D2 | ~~Overtime day-bucketing: server-local or `report_tz`?~~ | Phase 9 | **DECIDED 2026-09-20 — `report_tz`, corrected.** Server-local was never a reproducible target: the legacy inherits the host's php.ini while Laravel pins UTC, so "replicate exactly" would have meant replicating z.com's timezone. `report_tz` is the clock the pay run and `daily_series()` already use, and the column exists because this same bug was fixed one layer up. Restate with `overtime:recompute` (`monitoring.md` §4) |
